<div class="space-y-6 py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">{{ __('Dental care station') }}</h1>
            <p class="mt-1 text-sm text-gray-600">{{ __('Record findings, treatment performed, and referrals. Patients are sent to counselling when you save.') }}</p>
        </div>
        ...
        <div class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-4">
            <aside class="lg:col-span-1 space-y-3" wire:poll.10s>
                <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">{{ __('Awaiting dental clinic') }}</h2>
                ...
                @else
                    <ul class="space-y-2" role="list">
                        @foreach ($queueInterventions as $qi)
                            <li wire:key="dental-q-{{ $qi->getKey() }}">
                                <button
                                    type="button"
                    ...
                    <div class="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm space-y-6">
                        ...
                        @if (! $selectedIntervention)
                            <div class="rounded-md bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-900">
                                {{ __('This visit has no dental-care line queued (add Dental Care at vitals).') }}
                            </div>
                        @else
                            <div>
                            ...
                            @if (! $canRecord)
                                <div class="rounded-md bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-900">
                                    {{ __('This dental-care line is no longer editable from the queue.') }}
                                </div>
                            @else
                                <form
                                    wire:submit="saveExam"
                                    wire:confirm="{{ __('Save the dental exam and route this patient forward?') }}"
                                    class="space-y-6"
                                >
                                    @error('form')
                                    ...
                                    <div>
                                        <label for="dental-findings" class="block text-sm font-medium text-gray-700">{{ __('Findings') }} <span class="text-red-600">*</span></label>
                                        <textarea id="dental-findings" wire:model="examForm.findings" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary sm:text-sm"></textarea>
                                    ...
                                    <div>
                                        <label for="dental-treatment" class="block text-sm font-medium text-gray-700">{{ __('Treatment performed') }}</label>
                                        <textarea id="dental-treatment" wire:model="examForm.treatment_performed" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary sm:text-sm"></textarea>
                                        @error('examForm.treatment_performed')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <fieldset class="space-y-3 rounded-lg border border-gray-200 p-4">
                                        ...
                                        @if ($examForm['referral_needed'])
                                            <div>
                                                <label for="dental-ref-notes" class="block text-xs font-medium text-gray-600">{{ __('Referral notes') }} <span class="text-red-600">*</span></label>
                                                <textarea id="dental-ref-notes" wire:model="examForm.referral_notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary sm:text-sm"></textarea>
                                    ...
                                    <div>
                                        <label for="dental-notes" class="block text-sm font-medium text-gray-700">{{ __('Exam notes') }}</label>
                                        <textarea id="dental-notes" wire:model="examForm.notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary sm:text-sm"></textarea>
